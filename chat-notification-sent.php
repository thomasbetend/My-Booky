...
if(empty($_SESSION)){
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['select-chat-user-id'])){
        ...
        $queryNotificationAlreadySent = 'SELECT n.id FROM notification n WHERE (source_user_id = ' . $_SESSION['id'] . ' AND user_id = ' . $userId . ' AND accepted_by_source_user_id=true) OR (source_user_id = ' . $userId  . ' AND user_id = ' . $_SESSION['id'] . ' AND accepted_by_user_id=true)';
        ...
        if($notificationAlreadySent){ ?>
            ...
            <div class="container w-50 text-center">
                <h3 class="text-center mt-5">Notification déjà envoyée ou reçue</h3>
